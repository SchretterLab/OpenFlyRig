classdef LEDController < handle
    ...
    methods
        function obj = LEDController(COMPort)
            ...
            obj.dispstatus = 0;
            % Open the LED-controller serial port only if it exists. On the
            % open rig no controller is connected, so a missing/absent COM
            % port must NOT crash: fall back to serialPort = 0, which every
            % method below already treats as a safe no-op.
            try
