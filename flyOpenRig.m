function varargout = flyOpenRig(varargin)
% FLYBUBBLE_RGB MATLAB code for flyOpenRig.fig
...
%      applied to the GUI before flyOpenRig_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to flyOpenRig_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help flyOpenRig
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @flyOpenRig_OpeningFcn, ...
    'gui_OutputFcn',  @flyOpenRig_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before flyOpenRig is made visible.
function flyOpenRig_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to flyOpenRig (see VARARGIN)

% Choose default command line output for flyOpenRig
handles.output = hObject;

flyOpenRig_user_setting;

hComm = initialize_flyOpenRig;
handles.hComm = hComm;
...
% UIWAIT makes flyOpenRig wait for user response (see UIRESUME)
...
function varargout = flyOpenRig_OutputFcn(hObject, eventdata, handles)
...
try
    ...
    clearup_flyOpenRig(handles.hComm)
...
% Open rig: there is no LED controller, so the protocol is NOT uploaded to any
% hardware. The protocol .xlsx is used only to define the experiment duration
% (handles.totalDuration, set above) and to plot the protocol on protocol_axes.
% The original upload block (removeAllExperimentSteps / addOneStep /
% getExperimentSteps) is intentionally omitted: those calls return unassigned
% outputs when no controller is connected, which would error and disable Start.
...
if button_state == get(hObject,'Max')
    ...
    % Open rig: all 4 metadata tabs still display in the GUI, but only tab 1's
    % data is saved, so a single output folder is created per experiment.
    for i = 1:1
    ...
    % Open rig: no LED controller, so MATLAB itself is the experiment clock.
    % Capture the reference time at the moment recording starts; the experiment
    % callback measures elapsed time from this. runExperiment() is a harmless
    % no-op with no controller connected but kept for parity.
    handles.hComm.LEDCtrl.runExperiment();
    handles.expClockStart = datetime('now');
    guidata(hObject, handles);
    start(handles.tExp);
...
if handles.expRun == 1
    ...
    % Open rig: no LED controller to report elapsed time or LED levels, and no
    % divider hardware to fire. Measure elapsed time (seconds) from the
    % wall-clock reference captured when recording started.
    time = seconds(datetime('now') - handles.expClockStart);
...
if ~(handles.hComm.flea3{1} == 0) && handles.hComm.flea3IsActive(1)
    ...
    if ~isempty(D)
        movieFileWithVer = fullfile(handles.expDataSubdir1, D.name);
    end

    if exist(movieFileWithVer, 'file')
        % Open rig saves a single folder, so just normalize the movie name in
        % subdir 1 (no replication into the other three folders).
...
for i = 1:1
    ...
    userSettingMFile = [handles.defaultDir, 'flyOpenRig_user_setting.m'];
    userSettingTxtFile = [handles.(['expDataSubdir',num2str(i)]),'\flyOpenRig_user_setting.txt'];
...
for i = 1:1
    ...
    dataPath = [handles.tempPath1, '\', datestr(handles.expStartTime,30), '_', 'rig', handles.enclosureNum{i},...
        '_','flyOpenRig', num2str(i),'_',myData.mParent{i}, '_', myData.fParent{i},...
...
handles = guidata(hFig);
% Open rig: all 4 tabs are shown, but only tab 1's metadata is saved to the
% single output folder (and only tab 1's tree is persisted as last-used values).
for i = 1:1
        %save the meta data file
