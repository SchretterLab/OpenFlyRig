%flyOpenRig upper left corner
% OPTIONAL on the open rig: no LED backlight controller is required. If nothing
% is connected on this port, LEDController falls back to a no-op safely, so this
% value is only used if a controller happens to be present.
serial_port_for_LED_Controller = 'COM5';
% Set defaultDir to the folder where FlyOpenRig is installed on this rig. It must
% contain bias_config.json, bias_gui.bat, ledProtocols\, and flyBubbleMetaTree*.xml.
defaultDir = 'C:\Users\labadmin\Documents\MATLAB\FlyOpenRig\';
...
% OPTIONAL on the open rig: no precon temperature/humidity sensor is required.
% This port is not opened by initialize_flyOpenRig; the value is kept only for
